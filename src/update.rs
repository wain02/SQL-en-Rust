...
pub fn comando_update(consulta_del_terminal: String){
    ...
    let mut tabla_ingresada = &condiciones_separadas[0] ;
    ...
    update_csv(vector_condiciones, vector_filtros, tabla_de_consulta);
    
}

pub fn update_csv(vec_update: Vec<String>, vec_filtro: Vec<String> ,tabla_update: String)-> io::Result<()>{
    let mut columna_update =  &vec_update[0]; //email
    let mut valor_update =  &vec_update[1]; //mrodriguez@hotmail.com
    let mut columna_filtro =  &vec_filtro[0]; //id
    let mut valor_filtro =  &vec_filtro[1]; //4

    let mut index_filtro = 0; //0
    let mut index_update = 0; //3


    let mut  input = BufReader::new(fs::File::open(&tabla_update)?);
